...
def wholeFunction():
  ...
  for m in range(2, number_of_rows):
      sheet2.cell(row=m, column=20).value = sheet2.cell(row=m, column=8).value / sheet2.cell(row=(number_of_rows), column=8).value
      sheet2.cell(row=m, column=20).value = float(sheet2.cell(row=m, column=20).value) * float(Quantity_TOTAL_EGRESS_GB)
  ...
  for m in range(2, number_of_rows):
        if(sheet2.cell(row=m, column=10).value == '0.25 IOPS/GB'): #0.25
          sheet2.cell(row=m, column=23).value = sheet2.cell(row=m, column=21).value / Total_Calculos_Storage_025 
          sheet2.cell(row=m, column=23).value = float(sheet2.cell(row=m, column=23).value) * float(Quantity_TOTAL_STORAGE_POINT_TWO_FIVE_IOPS_GB_HOURS)
          Total_Precio_Final_Storage_025 = Total_Precio_Final_Storage_025 + sheet2.cell(row=m, column=23).value
        elif (sheet2.cell(row=m, column=10).value == '4 IOPS/GB'): #4
          sheet2.cell(row=m, column=26).value = sheet2.cell(row=m, column=24).value / Total_Calculos_Storage_4 
          sheet2.cell(row=m, column=26).value = float(sheet2.cell(row=m, column=26).value) * float(Quantity_TOTAL_STORAGE_FOUR_IOPS_GB_HOURS)
          Total_Precio_Final_Storage_4 = Total_Precio_Final_Storage_4 + sheet2.cell(row=m, column=26).value
        elif (sheet2.cell(row=m, column=10).value == '2 IOPS/GB'): #2
          sheet2.cell(row=m, column=29).value = sheet2.cell(row=m, column=27).value / Total_Calculos_Storage_2 
          sheet2.cell(row=m, column=29).value = float(sheet2.cell(row=m, column=29).value) * float(Quantity_TOTAL_STORAGE_TWO_IOPS_GB_HOURS)
          Total_Precio_Final_Storage_2 = Total_Precio_Final_Storage_2 + sheet2.cell(row=m, column=29).value
        elif (sheet2.cell(row=m, column=10).value == '10 IOPS/GB'): #10
          sheet2.cell(row=m, column=32).value = sheet2.cell(row=m, column=30).value / Total_Calculos_Storage_10 
          sheet2.cell(row=m, column=32).value = float(sheet2.cell(row=m, column=32).value) * float(Quantity_TOTAL_STORAGE_TEN_IOPS_GB_HOURS)
  ...
  for m in range(2, number_of_rows):
      if('Microsoft Windows ' in sheet2.cell(row=m, column=2).value): #windows
        sheet2.cell(row=m, column=35).value = float(sheet2.cell(row=m, column=34).value) * float(Quantity_TOTAL_WINDOWS_LICENSES_HOURS)
        Total_Precio_Final_Licencias_Windows = Total_Precio_Final_Licencias_Windows + sheet2.cell(row=m, column=35).value
      elif('Red Hat' in sheet2.cell(row=m, column=2).value and sheet2.cell(row=m, column=7).value <= 4): #Red Hat
        sheet2.cell(row=m, column=38).value = float(sheet2.cell(row=m, column=37).value) * float(Quantity_TOTAL_RHEL_SMALL_LICENSES)
        Total_Precio_Final_Licencias_RHS = Total_Precio_Final_Licencias_RHS + sheet2.cell(row=m, column=38).value
      elif('Red Hat' in sheet2.cell(row=m, column=2).value and sheet2.cell(row=m, column=7).value > 4): #windows
        sheet2.cell(row=m, column=41).value = float(sheet2.cell(row=m, column=40).value) * float(Quantity_TOTAL_RHEL_LARGE_LICENSES)
